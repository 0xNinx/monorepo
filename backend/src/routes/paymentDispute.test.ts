 ...
 *
 * createPaymentDisputeRouter() is never mounted in app.ts — these endpoints are
 * unreachable in the running application today. The live dispute flow users
 * actually hit is GET/POST /api/tenant/payments/disputes in tenantPayments.ts,
 * which has no duplicate-pending guard and, critically, has no resolve
